...
pub struct Input {
    ...
    /// Little endian key type that stores the index of the unspent output of the corresponding
    /// `previous_tx_id`. Required in PSBTv2 and must be excluded in PSBTv0.
    pub output_index: Option<u32>,
    /// Represents the sequence number of this input. Must be excluded in PSBTv0 and optional
    /// in PSBTv2. If not specified in PSBTv2, the default sequence number `0xFFFFFFFF` is assumed.
    /// Optional in PSBTv2, but not allowed in PSBTv0.
...
impl Input {
    ...
    pub fn combine(&mut self, other: Self) {
        ...
        if let (&None, Some(locktime)) = (&self.time_lock_time, other.time_lock_time) {
            self.time_lock_time = Some(locktime);
        }

        if let (&None, Some(locktime)) = (&self.height_lock_time, other.height_lock_time) {
            self.height_lock_time = Some(locktime);
        }
...
impl Map for Input {
    fn get_pairs(&self) -> Vec<raw::Pair> {
        ...
        impl_psbt_get_pair! {
            rv.push(self.time_lock_time, PSBT_IN_REQUIRED_TIME_LOCKTIME)
        }

        impl_psbt_get_pair! {
            rv.push(self.height_lock_time, PSBT_IN_REQUIRED_HEIGHT_LOCKTIME)
        }
